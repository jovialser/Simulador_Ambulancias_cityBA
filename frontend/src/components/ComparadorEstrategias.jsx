...
useEffect(() => {
  async function comparar() {
    ...
    try {
      ...
      if (!resTrad.ok) throw new Error(`Tradicional falló (${resTrad.status})`);
      const datosTrad = await resTrad.json();
      console.log("✅ Respuesta tradicional:", datosTrad);
      setTradicional(datosTrad);
    } catch (err) {
      console.error("❌ Error en solicitud tradicional:", err);
    }
    ...
    try {
      ...
      if (!resIA.ok) throw new Error(`IA falló (${resIA.status})`);
      const datosIA = await resIA.json();
      console.log("✅ Respuesta IA:", datosIA);
      setInteligente(datosIA);
    } catch (err) {
      console.error("❌ Error en solicitud IA:", err);
    }
  }
